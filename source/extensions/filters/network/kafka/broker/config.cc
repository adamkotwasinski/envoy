...
public:
  void dr_cb(RdKafka::Message& message) {
    if (message.err())
      ENVOY_LOG(trace, "Message delivery failed: {}", message.errstr());
    else
      ENVOY_LOG(trace, "Message delivered: {}", message.partition());
  }
};
...
Network::FilterFactoryCb KafkaConfigFactory::createFilterFactoryFromProtoTyped(
  ...
  if (producer) {
    ENVOY_LOG(trace, "producer is alive");
  }
  ...
  ENVOY_LOG(trace, "p_res = {}", p_res);

  ENVOY_LOG(trace, "poll start");
  producer->poll(10000);
  ENVOY_LOG(trace, "poll finished");
