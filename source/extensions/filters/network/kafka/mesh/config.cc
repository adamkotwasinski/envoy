...
Network::FilterFactoryCb KafkaMeshConfigFactory::createFilterFactoryFromProtoTyped(
  ...
  */
  ...
  const MagicSharedPtr magic = std::make_shared<Magic>(config);
  const UpstreamKafkaFacadeSharedPtr upstream_kafka_facade = std::make_shared<UpstreamKafkaFacade>(context.threadLocal(), context.api().threadFactory());
