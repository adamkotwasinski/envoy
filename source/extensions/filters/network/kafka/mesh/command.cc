...
void ApiVersionsRequestHolder::invoke(UpstreamKafkaFacade&, const ClusteringLogic&) {
...
MetadataRequestHolder::MetadataRequestHolder(AbstractRequestListener& filter, const std::shared_ptr<Request<MetadataRequest>> request):
    filter_{filter}, request_{request}
{
}

void MetadataRequestHolder::invoke(UpstreamKafkaFacade&, const ClusteringLogic&) {
  // Metadata requests are immediately ready for answer.
  filter_.onRequestReadyForAnswer();
}
...
AbstractResponseSharedPtr MetadataRequestHolder::computeAnswer() const {
  ...
  MetadataResponseBroker broker = {0, "127.0.0.1", 19092};
  std::vector<MetadataResponseTopic> response_topics;
  if (request_->data_.topics_) {
    for (const auto topic : *(request_->data_.topics_)) {
      MetadataResponsePartition partition = { 0, 0, 0, {broker.node_id_}, {broker.node_id_}};
      MetadataResponseTopic response_topic = { 0, topic.name_, false, { partition }};
...
void ProduceRequestHolder::invoke(UpstreamKafkaFacade& kafka_facade, const ClusteringLogic& clustering_logic) {
  for (auto& fm : footmarks_) {
    const std::string target_cluster = clustering_logic.computeClusterForTopic(fm.topic_);
    KafkaProducerWrapper& producer = kafka_facade.getProducer(target_cluster);
    RdKafka::ErrorCode ec = producer.send(shared_from_this(), fm.topic_, fm.partition_, fm.key_, fm.value_);
    if (RdKafka::ErrorCode::ERR_NO_ERROR != ec) {
      expected_responses_--;
      fm.error_code_ = ec;
    }
  }
