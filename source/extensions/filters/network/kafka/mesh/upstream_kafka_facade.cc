...
public:
  ThreadLocalKafkaFacade(const ClusteringConfiguration& clustering_configuration, Event::Dispatcher& dispatcher, Thread::ThreadFactory& thread_factory);
...
ThreadLocalKafkaFacade::ThreadLocalKafkaFacade(const ClusteringConfiguration& clustering_configuration, Event::Dispatcher& dispatcher, Thread::ThreadFactory& thread_factory):
    clustering_configuration_{clustering_configuration}, dispatcher_{dispatcher}, thread_factory_{thread_factory}
{
}

KafkaProducerWrapper& ThreadLocalKafkaFacade::getProducerForTopic(const std::string& topic) {
  const absl::optional<ClusterConfig> cluster_config = clustering_configuration_.computeClusterConfigForTopic(topic);
  if (cluster_config) {
    ...
    return (cluster_to_kafka_client_.end() == it) ? registerNewProducer(*cluster_config) : *(it->second);
  } else {
    throw EnvoyException(absl::StrCat("cannot compute target producer for topic: ", topic));
  }
}

KafkaProducerWrapper& ThreadLocalKafkaFacade::registerNewProducer(const ClusterConfig& cluster_config) {
  ENVOY_LOG(info, "Register new Kafka producer for cluster [{}]", cluster_config.name_);
  KafkaProducerWrapperPtr new_producer = std::make_unique<KafkaProducerWrapper>(dispatcher_, thread_factory_, cluster_config.upstream_producer_properties_);
  auto result = cluster_to_kafka_client_.emplace(cluster_config.name_, std::move(new_producer));
  return *(result.first->second);
}

UpstreamKafkaFacade::UpstreamKafkaFacade(const ClusteringConfiguration& clustering_configuration, ThreadLocal::SlotAllocator& tls_allocator, Thread::ThreadFactory& thread_factory):
    tls_{tls_allocator.allocateSlot()} {

  ThreadLocal::Slot::InitializeCb cb =
    [&clustering_configuration, &thread_factory](Event::Dispatcher& dispatcher) -> ThreadLocal::ThreadLocalObjectSharedPtr {
    return std::make_shared<ThreadLocalKafkaFacade>(clustering_configuration, dispatcher, thread_factory);
  };
  tls_->set(cb);
}
