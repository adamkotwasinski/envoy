...
KafkaProducerWrapper::KafkaProducerWrapper(
    Event::Dispatcher& dispatcher, Thread::ThreadFactory& thread_factory, const RawKafkaProducerConfig& configuration):
  ...
  for (const auto& e : configuration) {
    if (conf->set(e.first, e.second, errstr) != RdKafka::Conf::CONF_OK) {
      throw EnvoyException(absl::StrCat("Could not set producer property [", e.first, "] to [", e.second, "]:", errstr));
    }
  }
...
KafkaProducerWrapper::~KafkaProducerWrapper() {
  ...
  // Impl note: this could be optimized by having flag flipped by owning Facade for all of its clients so shutdowns happen in parallel.
  poller_thread_active_ = false;
  poller_thread_->join();
  ENVOY_LOG(warn, "Worker thread shut down successfully");
}

void KafkaProducerWrapper::send(
    const ProduceFinishCbSharedPtr origin, const std::string& topic, const int32_t partition, const absl::string_view key, const absl::string_view value) {
  {
    ...
    RdKafka::ErrorCode ec = producer_->produce(topic, partition, flags, value_data, value.size(), key.data(), key.size(), 0, NULL, NULL);
    if (RdKafka::ERR_NO_ERROR == ec) {
      ...
      ENVOY_LOG(warn, "Produce failure [{}] while sending {} value-bytes to [{}/{}]", ec, value.size(), topic, partition);
...
void KafkaProducerWrapper::checkDeliveryReports() {
  while (poller_thread_active_) {
    // We are going to wait for 1000ms, returning when an event (message delivery) happens or producer is closed.
    // Unfortunately we do not have any ability to interrupt this call, so every destructor is going to take up to this much time.
    producer_->poll(1000);
  }
