...
struct ClusterConfig {
  ...
  // Impl note: this could be replaced with creating (shared?) AdminClient and having it reach out upstream to get configuration (or we could just send a correct request via codec). The response would need to be cached (as this data is frequently requested).
  ...
  // This map always contains entry with key 'bootstrap.servers', as this is the only mandatory producer property.
 ...
 * Impl note: current matching from topic to cluster is based on prefix matching but more complex rules could be added.
 */
