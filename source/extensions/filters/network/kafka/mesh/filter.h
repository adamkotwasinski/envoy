...
class KafkaMeshFilter : public Network::ReadFilter, public AbstractRequestListener, private Logger::Loggable<Logger::Id::kafka> {
public:
  KafkaMeshFilter(const ClusteringConfiguration& clustering_configuration, UpstreamKafkaFacade& upstream_kafka_facade);
  ~KafkaMeshFilter();
...
private:
  ...
  const ClusteringConfiguration& clustering_configuration_; // For test injection.
  ...
  std::vector<AbstractInFlightRequestSharedPtr> requests_in_flight_;
};
