...
class ThreadLocalFetchPurger : public FetchPurger,
...
ThreadLocalFetchPurger::ThreadLocalFetchPurger(Event::Dispatcher& dispatcher)
...
FetchPurgerImpl::FetchPurgerImpl(ThreadLocal::SlotAllocator& slot_allocator)
    : tls_{slot_allocator.allocateSlot()} {
  ThreadLocal::Slot::InitializeCb cb =
      [](Event::Dispatcher& dispatcher) -> ThreadLocal::ThreadLocalObjectSharedPtr {
    return std::make_shared<ThreadLocalFetchPurger>(dispatcher);
  };
  tls_->set(cb);
}
