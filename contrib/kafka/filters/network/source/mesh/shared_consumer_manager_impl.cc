      ...
      for (const int32_t partition : topic_and_partitions.second) {
        ...
        if (0 != stored_messages.size()) {
          ENVOY_LOG(info,
                    ...
                    callback->debugId(), stored_messages.size(), str(kp));
        }
...
void Store::removeCallbackWithoutLocking(
  ...
  ENVOY_LOG(info, "Removed callback {} in {} partitions, there are {} left", callback->debugId(),
            removed, remaining);
}

Store::~Store() {
  ENVOY_LOG(info, "Store DTOR 1");
  int cb_c = 0;
  int msg_c = 0;
  {
    absl::MutexLock lock(&callbacks_mutex_);
  ...
  {
    absl::MutexLock lock(&messages_mutex_);
    for (const auto& e : messages_waiting_for_interest_) {
      msg_c += e.second.size();
    }
  }
  ENVOY_LOG(info, "Store DTOR 2: {}/{}", cb_c, msg_c);
}
