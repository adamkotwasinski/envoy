...
FetchRequestHolder::FetchRequestHolder(AbstractRequestListener& filter,
    ...
    : BaseInFlightRequest{filter}, consumer_manager_{consumer_manager}, fetch_purger_{fetch_purger},
      request_{request}, dispatcher_{filter.dispatcher()}, filter_id_{filter.debugId()} {}

FetchRequestHolder::~FetchRequestHolder() { ENVOY_LOG(info, "Fetch {} DTOR", debugId()); }
...
CallbackReply FetchRequestHolder::receive(InboundRecordSharedPtr message) {
  ...
  if (!finished_) {
    ...
    if (current_messages < MINIMAL_MSG_CNT) {
      ENVOY_LOG(info, "Fetch request {} processed message (and wants more {}): {}", debugId(),
                current_messages, message->toString());
      return CallbackReply::ACCEPTED_AND_WANT_MORE;
    } else {
      ENVOY_LOG(info, "Fetch request {} processed message (and is finished with {}): {}", debugId(),
                current_messages, message->toString());
      // We have all we needed, we can finish processing.
      finished_ = true;
      cleanup(false);
      return CallbackReply::ACCEPTED_AND_FINISHED;
    }
  } else {
    ENVOY_LOG(info, "Fetch request {} rejected message: {}", debugId(), message->toString());
...
std::string FetchRequestHolder::toString() const {
  ...
  oss << "[" << filter_id_ << "/" << request_->request_header_.correlation_id_ << "]";
  return oss.str();
}
