...
void FetchRecordConverterImpl::print(const Bytes& arg, std::string ctx) const {
...
FetchRecordConverterImpl::convert(const InboundRecordsMap& arg) const {
  ...
  for (const auto& partition_and_records : arg) {
    ...
    for (const auto& record : partition_records) {
      // ENVOY_LOG(info, "processing record {}-{} / {}", record->topic_name(), record->partition(),
      // record->offset());
  ...
  compaction) bit 7~15: unused lastOffsetDelta: int32 / 15 baseTimestamp: int64 / 23 maxTimestamp:
  int64 / 31 producerId: int64 / 39 producerEpoch: int16 / 41 baseSequence: int32 / 45 records:
  [Record]
  */
  for (auto& partition_and_records : partition_to_bytes) {
    ...
    int32_t last_offset_delta =
        htobe32(-1); /* We always claim that we are at the beginning of partition. */
    ...
    // ENVOY_LOG(info, "there are {} records for {}-{}, last offset is {}", be32toh(record_count),
    // kp.first, kp.second, be32toh(last_offset_delta));
    ...
    uint32_t magic_offset =
        sizeof(base_offset) + sizeof(batch_len) + sizeof(int32_t /* PL epoch */);
    out[magic_offset] = 2;
  ...
  for (const auto& partition_and_records : partition_to_bytes) {
    ...
    const auto frrpd = FetchResponseResponsePartitionData{
        partition, error_code, high_watermark, absl::make_optional(partition_and_records.second)};
...
void FetchRecordConverterImpl::append(Bytes& out, const InboundRecordSharedPtr& ptr) const {
  ...
  // offsetDelta: varint
  const int32_t offset_delta = ptr->offset_;
  ...
  */
  ...
  // ENVOY_LOG(info, "len of record {}-{}/{} = {}", ptr->topic_name(), ptr->partition(),
  // ptr->offset(), written);
  ...
  // ENVOY_LOG(info, "final len of record {}-{}/{} = {}", ptr->topic_name(), ptr->partition(),
  // ptr->offset(), b_out.length());
  std::ostringstream oss;
  oss << "[";
