...
std::vector<InboundRecordSharedPtr> RichKafkaConsumer::receiveRecordBatch() {
  ...
  if (RdKafka::ERR_NO_ERROR == message->err()) {
    ...
    // There could be something left in the buffer, so we try to drain it by consuming without
    // waiting. See: https://github.com/edenhill/librdkafka/discussions/3897
